...
int main(int argc,char* argv[])
{
  ...
  //如果是服务器程序不去 bind ，就会导致每次启动服务器的端口发生变化
  //客户端就没法连接了
  //如果客户端也 bind 了，可能会出现问题
  //通常情况下，一个端口号只能被一个进程 bind 
  ...
  while(1)
  {
    ...
    recvfrom(sock,buf_output,sizeof(buf_output)-1,0,NULL,NULL);//后两个地址填成NULL，表示不关注对端的地址
