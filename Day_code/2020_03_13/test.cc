    ...
    public:
        int getFirstUnFormedNum(std::vector<int> &arr, int length) 
        {
            ...
            for(i = 0; i < length; i++)
            {
                ...
                //{2， 3， 5}
                //i=0--d[10]=2 d[9]=2 d[8]=2 d[7]=2...d[2]=2 //i=1--d[10]=5 d[9]=5...d[5]=5 d[4]=3 d[3]=3 
                //i=2--d[10]=10 d[9]=8 d[8]=8 d[7]=7 d[6]=5 d[5]=5 
                for(j = sum; j >= arr[i]; j--)
                {
                    //逆序判断背包承重中能够放入的数据 //当数组中只有2的时候，背包承重从2-10都可以放入2的数值
                    ...
                    if (dp[j] < dp[j - arr[i]] + arr[i])//对每个承重计算当前最大能放置重量
                        dp[j] = dp[j - arr[i]]+ arr[i];//更新背包中能够放入的最大值 else
                    dp [j] = dp[j];
...
int main()
{
    ...
    while(length != 0)
    {
       ...
       cout << A.getFirstUnFormedNum(arr,length) << endl;
    }
    return 0;
}
